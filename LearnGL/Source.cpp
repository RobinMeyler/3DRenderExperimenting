...
int main()
{
	...
	Shader lightingShader("vertexLight.txt", "FragLight.txt");
	Shader lampShader("vertexLightother.txt", "FragLightother.txt");

	lightingShader.use(); // don't forget to activate the shader before setting uniforms!  


	unsigned int VBO, CubeVAO;		 // Stores large number of vertices in GPU memory.

	glGenVertexArrays(1, &CubeVAO);	// Can generate many Arrays and Buffers at once this way
	glGenBuffers(1, &VBO);		// 1 = how many you are making, you can have many.	
	...
	glGenVertexArrays(1, &lightVAO);	// Can generate many Arrays and Buffers at once this way
...
//		float camZ = cos(glfwGetTime()) * radius;
		...
		model = glm::translate(model, rotatingCamera);
